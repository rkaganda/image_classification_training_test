...
def predict_image(image):
    image_tensor = test_transforms(image).float()  # transform the image
    image_tensor = image_tensor.unsqueeze_(0)  # reshape the tensor
    input = Variable(image_tensor)  # create to store image
    input = input.to(device) # feed our input
    output = model(input)  # get the result
    index = output.data.cpu().numpy().argmax()  # fancy math to get the probability
    return index
...
for ii in range(len(images)):  # for each image
    ...
    sub = fig.add_subplot(1, len(images), ii+1)  # add a sublot that shows the image
